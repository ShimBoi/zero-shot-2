...
import time
from improve.custom.schedulers.basic_schedules import create_linear_schedule
import jax
...
class PPO(Agent):
    def __init__(
        ...
        if self._learning_rate_scheduler == "linear":
            total_updates = cfg["timesteps"] // cfg["rollouts"] * cfg["learning_epochs"] * cfg["mini_batches"]
            self.lr_schedule = create_linear_schedule(
                init_lr=self._learning_rate,
                total_steps=total_updates,
                end_lr=0.0
            )
        # elif self._learning_rate_scheduler is KLAdaptiveRL:
        #     self.lr_schedule = KLAdaptiveRL(**cfg["learning_rate_scheduler_kwargs"]).schedule()
        else:
            self.lr_schedule = self._learning_rate
    ...
    def record_transition(
        ...
        if self.memory is not None:
            ...
            if self._time_limit_bootstrap:
                rewards += self._discount_factor * values.reshape(-1, 1) * truncated
    ...
    def calc_avg_reward(self, memory):
        ...
        # Find episode boundaries for each environment
        episode_returns = []
        ...
        for env_idx in range(rewards.shape[1]):
            ...
            if len(done_indices) == 0:
                continue  # No completed episodes in this env
            
            start_idx = 0
            for done_idx in done_indices:
                # Sum rewards from start_idx to done_idx (inclusive)
                episode_reward = np.sum(env_rewards[start_idx:done_idx + 1])
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        print("-----------------------------------------------------------------------------------------------")
        print(f"Actions (Min): {actions.min(axis=(0, 1))}, Max: {actions.max(axis=(0, 1))}, Mean: {actions.mean(axis=(0, 1))}")
        print(f"Values (Min): {values.min():.6f}, Max: {values.max():.6f}, Mean: {values.mean():.6f}")
        print(f"Rewards (Min): {rewards.min():.6f}, Max: {rewards.max():.6f}, Mean: {rewards.mean():.6f}")
        print(f"Returns (Min): {returns.min():.6f}, Max: {returns.max():.6f}, Mean: {returns.mean():.6f}")
        print("-----------------------------------------------------------------------------------------------")
        ...
        ds = self.memory.sample_all(sequence_length=1,
                                        ...
                                        is_image_obs=False,
                                        epochs=self._learning_epochs)

        policy_loss = critic_loss = kl_div = std_dev = 0.0
        for batch in ds:
            ...
            kl_div += metrics_update["kl_divergence"]
            std_dev += metrics_update["stddev"]

            logp = metrics_update["logp"]
            sampled_log_prob = batch['log_prob']
            

        policy_loss /= self._learning_epochs * self._mini_batches
        critic_loss /= self._learning_epochs * self._mini_batches
        kl_div /= self._learning_epochs * self._mini_batches
        std_dev /= self._learning_epochs * self._mini_batches
        ...
        self.track_data("Policy / Log probs (mean)", logp.mean())
        self.track_data("Policy / Log probs (min)", logp.min())
        self.track_data("Policy / Log probs (max)", logp.max())
        self.track_data("Policy / KL divergence", kl_div)
        self.track_data("Policy / Ratio (mean)", jnp.exp(logp - sampled_log_prob).mean())
        self.track_data("Policy / Ratio (min)", jnp.exp(logp - sampled_log_prob).min())
        self.track_data("Policy / Ratio (max)", jnp.exp(logp - sampled_log_prob).max())
        self.track_data("Loss / Policy loss", policy_loss)
        self.track_data("Loss / Value loss", critic_loss)
        self.track_data("Policy / Standard deviation", std_dev)
        
        if self._learning_rate_scheduler:
            self.track_data("Learning / Learning rate", self.lr_schedule(timestep))