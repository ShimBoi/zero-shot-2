...
class Agent:
    def __init__(
        ...
        self.write_interval = self.cfg.get(
            "experiment", {}).get("write_interval", "auto")
        ...
        self.checkpoint_interval = self.cfg.get(
            "experiment", {}).get("checkpoint_interval", "auto")
        self.checkpoint_store_separately = self.cfg.get(
            "experiment", {}).get("store_separately", False)
        self.checkpoint_best_modules = {
            "timestep": 0, "reward": -(2**31), "saved": False, "modules": {}}

        # experiment directory
        directory = self.cfg.get("experiment", {}).get("directory", "")
        experiment_name = self.cfg.get(
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            ...
            wandb_kwargs.setdefault(
                "name", os.path.split(self.experiment_dir)[-1])
        ...
        if self.write_interval > 0:
                ...
                try:
                    ...
                    class _SummaryWriter:
                        def __init__(self, log_dir):
                            self.writer = tensorflow.summary.create_file_writer(
                                logdir=log_dir)
            ...
            if self.writer is None:
                logger.warning(
                    "No package found to write events to Tensorboard.")
                logger.warning(
                    "Set agent's `write_interval` setting to 0 to disable writing")
                ...
                logger.warning(
                    "  - TensorboardX: https://github.com/lanpa/tensorboardX#install")
                logger.warning(
                    "The current running process will be terminated.")
                exit()

        if self.checkpoint_interval == "auto":
            self.checkpoint_interval = int(
                trainer_cfg.get("timesteps", 0) / 10)
        if self.checkpoint_interval > 0:
            os.makedirs(os.path.join(self.experiment_dir,
    ...
    def record_transition(
        ...
        if self.write_interval > 0:
            ...
            if self._cumulative_rewards is None:
                self._cumulative_rewards = np.zeros_like(
                    rewards, dtype=np.float32)
                self._cumulative_timesteps = np.zeros_like(
                    rewards, dtype=np.int32)
            ...
            for name, data in modules.items():
                ...
                if module is not None:
                    if hasattr(module, "state_dict"):
                        ...
                        module.state_dict = module.state_dict.replace(
                            params=params)
                    else:
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        if timestep > 1 and self.checkpoint_interval > 0 and not timestep % self.checkpoint_interval:
            # update best models
            reward = np.mean(self.tracking_data.get(
                "Reward / Total reward (mean)", -(2**31)))
