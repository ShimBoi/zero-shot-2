    ...
    def _value_loss(params):
        ...
        if clip_predicted_values:
            predicted_values = sampled_values + \
                jnp.clip(predicted_values - sampled_values, -
                         value_clip, value_clip)
        return value_loss_scale * ((sampled_returns - predicted_values) ** 2).mean()

    value_loss, grad = jax.value_and_grad(
        _value_loss, has_aux=False)(value_state_dict.params)
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            self.memory.create_tensor(
                name="states", size=self.observation_space, dtype=jnp.float32)
            self.memory.create_tensor(
                name="actions", size=self.action_space, dtype=jnp.float32)
            self.memory.create_tensor(
            ...
            self._tensors_names = ["states", "actions",
                                   "log_prob", "values", "returns", "advantages"]
        ...
        for epoch in range(self._learning_epochs):
            ...
            if self._learning_rate_scheduler:
                if self._learning_rate_scheduler is KLAdaptiveLR:
                    ...
                    if config.jax.is_distributed:
                        kl = jax.pmap(lambda x: jax.lax.psum(
                            x, "i"), axis_name="i")(kl.reshape(1)).item()
                        kl /= config.jax.world_size
                    self._learning_rate = self.scheduler(
                        timestep, self._learning_rate, kl)
                else:
                    self._learning_rate *= self.scheduler(timestep)

        # record data
        self.track_data("Loss / Policy loss", cumulative_policy_loss /
                        (self._learning_epochs * self._mini_batches))
        self.track_data("Loss / Value loss", cumulative_value_loss /
                        (self._learning_epochs * self._mini_batches))
        if self._entropy_loss_scale:
            self.track_data(
