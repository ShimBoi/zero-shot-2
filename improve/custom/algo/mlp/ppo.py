...
def _update_all(
    ...
    def loss_fn(params):
        ...
        def _value_loss(params):
            ...
            _, _, outputs = model_act({"states": sampled_states}, "value", params)
        ...
        p_loss, (entropy_loss, kl_divergence, stddev) = _policy_loss(params)
        v_loss = _value_loss(params)
...
class PPO(Agent):
    def __init__(
        ...
        self.model = self.models.get("a2c", None)

        # checkpoint models
        # self.checkpoint_modules["policy"] = self.policy
        # self.checkpoint_modules["value"] = self.value
        self.checkpoint_modules["a2c"] = self.model
        ...
        if config.jax.is_distributed:
            ...
            if self.model is not None:
                self.model.broadcast_parameters()
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            self.memory.create_tensor(name="states", size=np.prod(self.observation_space.shape), dtype=jnp.float32)
            self.memory.create_tensor(name="actions", size=np.prod(self.action_space.shape), dtype=jnp.float32)
    ...
    def act(self, states: Union[np.ndarray, jax.Array], timestep: int, timesteps: int) -> Union[np.ndarray, jax.Array]:
        ...
        actions, log_prob, outputs = self.model.act({"states": self._state_preprocessor(states)}, role="policy")
    ...
    def record_transition(
        ...
        if self.memory is not None:
            ...
            # compute values
            _, _, outputs = self.model.act({"states": self._state_preprocessor(states)}, role="value")
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        # )  # TODO: .float()
        _, _, outputs = self.model.act({"states": self._state_preprocessor(self._current_next_states)}, role="value")
        ...
        ds = self.memory.sample_all(names=self._tensors_names, 
                                    sequence_length=1,
        ...
        for batch in ds:
            ...
            # optimization step (policy and value)
            if config.jax.is_distributed:
                grads = self.model.reduce_parameters(grads)
            self.optimizer = self.optimizer.step(
                grads, self.model, self._learning_rate if self._learning_rate_scheduler else None
            )
