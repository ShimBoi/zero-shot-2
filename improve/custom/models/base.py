...
class Model(flax.linen.Module):
    ...
    def init_state_dict(
        ...
        if not inputs:
            inputs = {
                "image": jnp.zeros((1, 15, 300, 300, 3)),
        ...
        with jax.default_device(self.device):
            params = self.init(key, inputs, None, None, act_tokens, train=self.training)
            self.state_dict = StateDict.create(apply_fn=self.apply, params=params)
